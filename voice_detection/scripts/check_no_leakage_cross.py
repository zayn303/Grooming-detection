...
if leaked:
    print(f"⚠️ Found {len(leaked)} duplicate entries across splits!")
    for h, splits in leaked.items():
        print(f" - Hash {h} appears in: {splits}")
else:
    print("✅ No data leakage detected between splits in metadata_cross.csv")
